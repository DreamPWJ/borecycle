...
//service在使用this指针，而factory直接返回一个对象
  ...
  .service('AccountService', function ($q, $http, BoRecycle, $cordovaFileTransfer, $state, $ionicScrollDelegate, $cordovaToast, $timeout, $ionicHistory, $ionicPopup, $ionicLoading, $cordovaFile, $cordovaFileOpener2) { //我的服务
    return {
      ...
      registernew: function (datas) { //用户注册
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise
        promise = $http({
          method: 'POST',
          url: BoRecycle.api + "/api/user/regnew",
          data: datas
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
      ...
      getInvitecode_id: function (params) {
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise
        promise = $http({
          method: 'GET',
          url: BoRecycle.api + "/api/invitecode/valid/" + params
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
          deferred.reject(err);// 声明执行失败，即服务器返回错误
      ...
      twoElementAuthenticate: function (datas) { //提交实名认证，需要带入authenticate_sign 实名认证服务id
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise
        promise = $http({
          method: 'POST',
          url: BoRecycle.api + "/api/user/authenticate_idcard",
          data: datas
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
  ...
  .service('OrderService', function ($q, $http, BoRecycle, $state, CommonService) { //订单 接单收货/货源归集及回收 登记信息/货源接口服务定义
    return {
      ...
      addappoint: function (datas) { //预约上门时间
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise;
        promise = $http({
          method: 'POST',
          url: BoRecycle.api + "/api/orderreceipt/appoint",
          data: datas
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
      ...
      getappoint: function (params) { //取接单时间后N天时间数据
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise;
        promise = $http({
          method: 'GET',
      ...
      getlistyj: function (params, datas) { //查询登记信息/货源信息分页列
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise;
        promise = $http({
          method: 'GET',
          url: BoRecycle.api + "/api/dengji/getlistyj/" + params.page + "/" + params.size,
          params: datas
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
      ...
      getgrplistnew: function (params) { //获取产品品类
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise;
        promise = $http({
          method: 'GET',
          cache: true,
          url: BoRecycle.api + "/api/product/getgrplistnew",
          params: params
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
  ...
  .service('PayService', function ($q, $http, BoRecycle, CommonService, $state) {
    return {
      ...
      alipaywapRecharge: function (datas) { //支付宝支付订单信息
        var deferred = $q.defer();// 声明延后执行，表示要去监控后面的执行
        var promise = deferred.promise
        promise = $http({
          method: 'POST',
          url: BoRecycle.api + "/api/aop/alipaywap",
          data: datas
        }).success(function (data) {
          deferred.resolve(data);// 声明执行成功，即http请求数据成功，可以返回数据了
        }).error(function (err) {
